...
description = "CR Onsrud 5-axis Router with Fanuc 31i control";
vendor = "C.R. Onsrud";
...
function setSmoothing(mode) {
  ...
  disableLengthCompensation(false); // required
  if (mode) {
    writeBlock(gFormat.format(5.1), "Q1", "R7");
  } else {
  ...
  if (insertToolCall || !lengthCompensationActive || retracted || (!isFirstSection() && getPreviousSection().isMultiAxis())) {
    ...
    if (!machineConfiguration.isHeadConfiguration()) {
      ...
      writeBlock(gMotionModal.format(0),
        gFormat.format(getProperty("fifthAxis") ? 43.4 : 43),
